...
- Η **κατηγορία της τράπεζας** είναι 2 επιπέδων (`Σπίτι / Supermarket`) → χρησιμοποιείται ως
  πρόταση **μόνο αν η κατηγορία υπάρχει ήδη** (δεν δημιουργούνται κατηγορίες αυτόματα — ο χρήστης
  τις φτιάχνει από τη σελίδα «Κατηγορίες»). Το `Χωρίς Κατηγορία` / `Ανακατανομή` → uncategorized.

### Συμπεριφορά import (v1.1)

- **Δεν** δημιουργούνται κατηγορίες αυτόματα από το Excel.
- Στο preview ο χρήστης μπορεί να **ξετσεκάρει** γραμμές ώστε να μην εισαχθούν.
- Αν το **παραστατικό υπάρχει ήδη** (ίδιο fingerprint, ή μοναδικό `Αριθμός Παραστατικού`), η κίνηση
  **ενημερώνεται** αντί να διπλοκαταχωρηθεί. Οι εντός-αρχείου επαναλήψεις παραλείπονται.
